...
const emptyPolicy = { title: '', description: '', is_active: true }
...
const Icon = {
  ...
  onboarding: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>,
  policy: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>,
  ...
  pdf: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>,
  ...
  xmark: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>,
...
function App() {
  ...
  return (
    <div className={darkMode ? 'app dark' : 'app'}>
      ...
      <Sidebar collapsed={collapsed} navItems={navItems} path={path} navigate={navigate} data={data} />
      <div className="workspace">
        <Topbar admin={admin} breadcrumbs={breadcrumbs[crumbKey] || ['Admin']} collapsed={collapsed} setCollapsed={setCollapsed} darkMode={darkMode} setDarkMode={setDarkMode} onLogout={handleLogout} loading={loading} onRefresh={loadData} />
        <main className="page">
...
/* ─────────────────────────────────────────────────────────── Sidebar */
function Sidebar({ collapsed, navItems, path, navigate, data }) {
...
/* ─────────────────────────────────────────────────────────── Topbar */
function Topbar({ admin, breadcrumbs, collapsed, setCollapsed, darkMode, setDarkMode, onLogout, loading, onRefresh }) {
  return (
...
function OnboardingFormPage({ selected, navigate, loadData }) {
  ...
  const submit = async (event) => {
    ...
    try {
      if (image) {
        // If image selected, use multipart
        const payload = new FormData()
        Object.entries(form).forEach(([k, v]) => payload.append(k, v ?? ''))
        payload.append('image', image)
        ...
        if (editing) await onboardingRepository.update(selected.id, form)
        else await onboardingRepository.save(form)
      }
      await loadData()
          ...
          <table>
            ...
            <tbody>
              {rows.length ? rows.map((policy) => (
                ...
                <tr><td colSpan={4} style={{ textAlign: 'center', color: 'var(--muted)', padding: 32 }}>No policies yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
